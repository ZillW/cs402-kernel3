...
read_args_t kern_args;
    ...
    status = copy_from_user(&kern_args, arg, sizeof(read_args_t));
    if (status < 0) {
    ...
    bytes_actually_read = do_read(kern_args.fd, temp_buffer, kern_args.nbytes);

    if (bytes_actually_read < 0) {
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
        curthr->kt_errno = -bytes_actually_read;
        page_free(temp_buffer);
        return -1;
    }

    dbg(DBG_PRINT, "(GRADING3D 1)\n");
    status = copy_to_user(kern_args.buf, temp_buffer, bytes_actually_read);
...
write_args_t kern_args;
        ...
        if (copy_from_user(&kern_args, arg, sizeof(write_args_t)) < 0) {
        ...
        if (copy_from_user(kernel_buffer, kern_args.buf, kern_args.nbytes) < 0) {
                page_free(kernel_buffer);
                curthr->kt_errno = EFAULT;
                return -1;
        }

        ret_status = do_write(kern_args.fd, kernel_buffer, kern_args.nbytes);

        if (ret_status < 0) {
                curthr->kt_errno = -ret_status;
                page_free(kernel_buffer);
                return -1;
        }
...
getdents_args_t kernel_args;
    ...
    int copy_in_status;
    int bytes_read_this_loop;
    int copy_out_status;
    size_t total_bytes_copied = 0;

    copy_in_status = copy_from_user(&kernel_args, arg, sizeof(getdents_args_t));
    if (copy_in_status < 0) {
        curthr->kt_errno = -copy_in_status;
        return -1;
    }

    while (total_bytes_copied + sizeof(dirent_t) <= kernel_args.count) {
        
        bytes_read_this_loop = do_getdent(kernel_args.fd, &kernel_dirent);
        ...
        if (bytes_read_this_loop < 0) {
            ...
            curthr->kt_errno = -bytes_read_this_loop;
            return -1;
        }

        dbg(DBG_PRINT, "(GRADING3B)\n");
        
        void *user_destination = (char *)kernel_args.dirp + total_bytes_copied;
        copy_out_status = copy_to_user(user_destination, &kernel_dirent, bytes_read_this_loop);

        if (copy_out_status < 0) {
            curthr->kt_errno = -copy_out_status;
            return -1;
        }
        
        total_bytes_copied += bytes_read_this_loop;
    }
