...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
    ...
    size_t actual_stack_size = DEFAULT_STACK_SIZE;;
...
kthread_clone(kthread_t *thr)
{
    ...
    kthread_t *cloned_thread = (kthread_t *)slab_obj_alloc(kthread_allocator);
    if (cloned_thread == NULL) {
        dbg(DBG_PRINT, "(GRADING3B)\n");
        return NULL;
    }

    cloned_thread->kt_kstack = alloc_stack();
    if (cloned_thread->kt_kstack == NULL) {
        slab_obj_free(kthread_allocator, cloned_thread);
        dbg(DBG_PRINT, "(GRADING3B)\n");
        return NULL;
    }

    cloned_thread->kt_retval = NULL;
    cloned_thread->kt_errno = 0;
    cloned_thread->kt_proc = NULL;
    cloned_thread->kt_cancelled = 0;
    cloned_thread->kt_wchan = NULL;
    cloned_thread->kt_state = KT_RUN;

    list_link_init(&cloned_thread->kt_qlink);
    list_link_init(&cloned_thread->kt_plink);

    KASSERT(cloned_thread->kt_state == KT_RUN);
    dbg(DBG_PRINT, "(GRADING3B)\n");
