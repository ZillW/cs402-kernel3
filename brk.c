...
if (addr == NULL) {
    ...
    uint32_t new_end_vfn = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
    uint32_t old_end_vfn = ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk));

    if (new_end_vfn > old_end_vfn) {
        
        dbg(DBG_PRINT, "(GRADING3B)\n");
        uint32_t pages_to_add = new_end_vfn - old_end_vfn;

        if (!vmmap_is_range_empty(curproc->p_vmmap, old_end_vfn, pages_to_add)) {
            dbg(DBG_PRINT, "(GRADING3E)\n");
            return -ENOMEM;
        }

        vmarea_t *heap_vma = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(curproc->p_start_brk));
	if (heap_vma == NULL) {
            dbg(DBG_PRINT, "(GRADING3E)\n");
            return -ENOMEM;
        }        
        
        heap_vma->vma_end = new_end_vfn;
        
    } else if (new_end_vfn < old_end_vfn) {

        dbg(DBG_PRINT, "(GRADING3B)\n");
        uint32_t pages_to_remove = old_end_vfn - new_end_vfn;
        
        vmmap_remove(curproc->p_vmmap, new_end_vfn, pages_to_remove);
